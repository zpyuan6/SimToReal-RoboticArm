...
def main() -> None:
    ...
    pseudo_cfg = cfg.get("pseudo_real", {})
    np.random.seed(cfg["seed"] + 211)
    env = RoArmSimEnv(cfg["sim"], seed=cfg["seed"] + 211)
    expert = ScriptedExpert()
    data_root = ensure_dir(cfg["paths"]["data_root"])
    split_sizes = {
        "calibration": int(pseudo_cfg.get("calibration_episodes", 120)),
        "heldout": int(pseudo_cfg.get("heldout_episodes", 180)),
    }
